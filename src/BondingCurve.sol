...
abstract contract BondingCurve is ERC20Upgradeable, BancorFormula {
  ...
  */
   ...
   */
  function _buy(uint256 buyAmount, address recipient) internal {
    require(buyAmount > 0);
    uint256 tokensToMint = calculatePurchaseReturn(totalSupply(), poolBalance, reserveRatio, buyAmount);
   ...
   * @param recipient the address receiving the sale return
   */
  function _sell(address from, uint256 sellAmount, address recipient) internal {
    require(sellAmount > 0 && balanceOf(from) >= sellAmount);
    uint256 amountOut = calculateSaleReturn(totalSupply(), poolBalance, reserveRatio, sellAmount);
